...
#ifdef ENABLE_VR
m64p_handle g_VrConfig = NULL;   /* [Core-VR] section; read by src/vr/* */
#endif
...
#ifdef ENABLE_VR
    /* VR (OpenXR head-tracking) — separate [Core-VR] section, both switches default off
     * so a VR-enabled build behaves exactly like upstream until explicitly turned on. */
    if (ConfigOpenSection("Core-VR", &g_VrConfig) == M64ERR_SUCCESS)
    {
        ConfigSetDefaultBool(g_VrConfig, "EnableVR", 0, "Render into a headset via OpenXR (Tier-1 virtual screen)");
        ConfigSetDefaultBool(g_VrConfig, "EnableHeadCamera", 0, "Inject head-tracked camera into the game (Tier-3); the user-global head-look toggle. A per-game VR profile's headLook overrides this in both directions.");
        /* User-global VR knobs — the base of the precedence chain (compiled default < profile <
         * THESE < per-game override). DEFAULTS MUST MATCH VR_DEFAULTS_INIT in src/vr/vr_profile.c
         * so an untouched knob reads back == its default and a profile may fill it. */
        ConfigSetDefaultFloat(g_VrConfig, "Separation", 0.012f, "Stereo separation / parallax budget (depth strength).");
        ConfigSetDefaultFloat(g_VrConfig, "Convergence", 0.0f, "Zero-parallax plane depth (raw clip-W units; per-game via profile).");
        ConfigSetDefaultFloat(g_VrConfig, "HeadLookSensitivity", 1.0f, "Head-look rotation gain.");
        ConfigSetDefaultFloat(g_VrConfig, "ScreenDistance", 2.0f, "Virtual screen distance from the viewer (metres).");
        ConfigSetDefaultFloat(g_VrConfig, "ScreenHeight", 1.4f, "Virtual screen physical height (metres).");
        ConfigSetDefaultFloat(g_VrConfig, "ScreenArc", 0.0f, "Cylindrical screen wrap (degrees; 0 = flat quad).");
        ConfigSetDefaultFloat(g_VrConfig, "ScreenVerticalOffset", 0.0f, "Screen vertical offset vs eye-height anchor (metres; -down).");
    }
#endif
...
void new_frame(void)
{
    ...
    /* MUPEN_VR_AUTOLOAD_STATE=<frame>: fire ONE state-load (current slot) once the
     * frame counter reaches <frame> (min 30, ~0.5 s — the machine snapshot restores
     * fine any time after emulation starts). Headless calibration and camera-hunt
     * verification need a way into a captured scene with no keyboard attached. */
    {
        static int autoload_frame = -2;             /* -2 unread, -1 off/fired */
        if (autoload_frame == -2) {
            const char* e = getenv("MUPEN_VR_AUTOLOAD_STATE");
            autoload_frame = (e != NULL && *e != '\0') ? (atoi(e) < 30 ? 30 : atoi(e)) : -1;
        }
        if (autoload_frame >= 0 && l_CurrentFrame >= autoload_frame) {
            autoload_frame = -1;
            main_state_load(NULL);                  /* NULL = current slot */
        }
    }
...
#ifdef ENABLE_VR
    /* Tier-3B: write the guest camera toward the head pose, once per VI, right
     * after the cheat/GS writes. Inert unless Core-VR[EnableHeadCamera] and a
     * matching per-game profile are active (and never during netplay). */
    vr_camera_apply(&g_dev.r4300);
#endif
...
#ifdef ENABLE_VR
    /* Load the running ROM's VR profile (keyed by ROM_SETTINGS.MD5) before the
     * plugins start — per-game head-look/stereo settings; no profile => all VR
     * injection stays inert for this game. */
    vr_profile_load_for_current_rom();
#endif
