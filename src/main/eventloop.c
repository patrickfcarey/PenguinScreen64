...
#include "util.h"
#ifdef ENABLE_VR
#include "vr/vr_camera.h"     /* vr_camera_request_recenter — head-look re-zero    */
#include "vr/vr_video_vk.h"   /* vr_video_vk_recenter     — screen anchor re-set  */
#endif
...
#ifdef ENABLE_VR
#define kbdVRRecenter "Kbd Mapping VR Recenter"
#define kbdVRFollow   "Kbd Mapping VR Follow Toggle"
#endif
...
#ifdef ENABLE_VR
                                   ,"Joy Mapping VR Recenter"
#endif
                                  };

static const int NumJoyCommands = sizeof(JoyCmdName) / sizeof(const char *);

static int JoyCmdActive[20][2];  /* if extra joystick commands are added above, make sure there is enough room in this array */
                                 /* [i][0] is Command Active, [i][1] is Hotkey Active */

#ifdef ENABLE_VR
/* One VR Recenter action, both consumers: re-anchor the virtual screen to the current
 * head pose (yaw + eye height) and re-zero the head-look compose reference. */
static void event_vr_recenter(void)
{
    vr_video_vk_recenter();
    vr_camera_request_recenter();
    DebugMessage(M64MSG_INFO, "VR recenter requested (screen anchor + head-look zero)");
}

/* F6: live A/B toggle of screen-follow (window tracks head yaw vs world-locked). */
static void event_vr_follow_toggle(void)
{
    int on = vr_video_vk_toggle_follow();
    DebugMessage(M64MSG_INFO, "VR screen follow: %s", on ? "ON (window tracks head)" : "OFF (world-locked)");
}

/* Multi-button recenter chord ("J0B4+B5+B9+B10" — '+'-separated, 2..4 buttons, one
 * device). The stock Jn<action>[/<hotkey>] parser tops out at TWO simultaneous
 * inputs; a controller-friendly recenter (LB+RB+L3+R3 on an Xbox pad, the pcsx2-VR
 * convention) needs four. This matcher is deliberately VR-only: '+' strings are
 * handled here, everything else still flows through MatchJoyCommand. Edge-triggered:
 * fires once when the last button of the set goes down, re-arms when any releases. */
#define VR_CHORD_MAX 4
static int  s_vr_chord_parsed = 0;              /* 0 unparsed, 1 active, -1 not-a-chord */
static int  s_vr_chord_dev = 0;
static int  s_vr_chord_n = 0;
static int  s_vr_chord_btn[VR_CHORD_MAX];
static int  s_vr_chord_down[VR_CHORD_MAX];
static int  s_vr_chord_fired = 0;

static void vr_chord_parse(void)
{
    const char* str = ConfigGetParamString(l_CoreEventsConfig, JoyCmdName[joyVRRecenter]);
    s_vr_chord_parsed = -1;
    s_vr_chord_n = 0;
    if (str == NULL || str[0] != 'J' || strchr(str, '+') == NULL)
        return;                                  /* stock parser's job (or unbound) */
    if (str[1] < '0' || str[1] > '9')
        return;
    s_vr_chord_dev = str[1] - '0';
    str += 2;
    while (*str != '\0' && s_vr_chord_n < VR_CHORD_MAX) {
        int btn;
        if (*str == '+')
            ++str;
        if (sscanf(str, "B%d", &btn) != 1)
            return;                              /* malformed -> disabled */
        s_vr_chord_btn[s_vr_chord_n]  = btn;
        s_vr_chord_down[s_vr_chord_n] = 0;
        ++s_vr_chord_n;
        while (*str != '\0' && *str != '+')
            ++str;
    }
    if (s_vr_chord_n >= 2) {
        s_vr_chord_parsed = 1;
        DebugMessage(M64MSG_INFO, "VR recenter chord armed: joystick %d, %d buttons (%s)",
                     s_vr_chord_dev, s_vr_chord_n,
                     ConfigGetParamString(l_CoreEventsConfig, JoyCmdName[joyVRRecenter]));
    }
}

/* Feed one SDL joystick button event; fires recenter on the all-held edge. */
static void vr_chord_event(const SDL_Event* event)
{
    int i, all;
    if (s_vr_chord_parsed == 0)
        vr_chord_parse();
    if (s_vr_chord_parsed != 1)
        return;
    if (event->type != SDL_JOYBUTTONDOWN && event->type != SDL_JOYBUTTONUP)
        return;
    if (event->jbutton.which != l_iJoyInstanceID[s_vr_chord_dev])
        return;
    for (i = 0; i < s_vr_chord_n; i++)
        if (event->jbutton.button == s_vr_chord_btn[i])
            s_vr_chord_down[i] = (event->type == SDL_JOYBUTTONDOWN) ? 1 : 0;
    all = 1;
    for (i = 0; i < s_vr_chord_n; i++)
        all &= s_vr_chord_down[i];
    if (all && !s_vr_chord_fired) {
        s_vr_chord_fired = 1;
        event_vr_recenter();
    } else if (!all) {
        s_vr_chord_fired = 0;
...
#ifdef ENABLE_VR
            vr_chord_event(event);   /* '+'-chord recenter (LB+RB+L3+R3); no-op for stock strings */
#endif
...
void event_initialize(void)
{
    ...
    for (i = 0; i < NumJoyCommands; i++)
        for (j = 0; j < 2; j++)
            JoyCmdActive[i][j] = 0;
#ifdef ENABLE_VR
    vr_chord_parse();   /* eager: logs "chord armed" at startup so a diag run shows the binding */
#endif
...
#ifdef ENABLE_VR
    ConfigSetDefaultInt(l_CoreEventsConfig, kbdVRRecenter, sdl_native2keysym(SDL_SCANCODE_F8), "SDL keysym for VR recenter (screen anchor + head-look zero)");
    ConfigSetDefaultInt(l_CoreEventsConfig, kbdVRFollow, sdl_native2keysym(SDL_SCANCODE_F6), "SDL keysym for VR screen-follow toggle (window tracks head yaw vs world-locked)");
    ConfigSetDefaultString(l_CoreEventsConfig, JoyCmdName[joyVRRecenter], "", "Joystick event string for VR recenter (stock J0B6 / J0B6/B4, or a 2-4 button chord J0B4+B5+B9+B10 = LB+RB+L3+R3 on an Xbox pad)");
#endif
#endif /* NO_KEYBINDINGS */
    return 1;
}
...
#ifdef ENABLE_VR
    else if (keysym == sdl_keysym2native(ConfigGetParamInt(l_CoreEventsConfig, kbdVRRecenter)))
        event_vr_recenter();
    else if (keysym == sdl_keysym2native(ConfigGetParamInt(l_CoreEventsConfig, kbdVRFollow)))
        event_vr_follow_toggle();
#endif
    else
#endif /* NO_KEYBINDINGS */
    {
