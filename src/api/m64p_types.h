...
/* ----------------------------------------- */
/* Structures and Types for VR (ENABLE_VR)   */
/* ----------------------------------------- */

/* A head pose sampled from the OpenXR runtime, in plain floats (no OpenXR types,
 * so consumers carry no OpenXR dependency and stay headless-testable). Orientation
 * is a unit quaternion in OpenXR component order (x,y,z,w); position is metres in
 * the runtime LOCAL (seated) space. valid==0 is the kill switch: consumers no-op.
 * Read by the video plugin via CoreVR_GetHeadPose(). See src/vr/vr_pose.h. */
typedef struct
{
   float orient_x, orient_y, orient_z, orient_w; /* unit quaternion (x,y,z,w) */
   float pos_x, pos_y, pos_z;                     /* metres, LOCAL space; unused in v1 */
   int      valid;                                /* 0 => consumers MUST no-op */
   uint32_t frame;                                /* monotonic publish counter; 0 = never */
} m64p_vr_pose;
