...
/* Do NOT "promote" this core's exports into the global scope (an earlier VR fix
 * re-dlopened ourselves here with RTLD_NOW|RTLD_GLOBAL|RTLD_NOLOAD so the video
 * plugin could reach the CoreVR_* handshake exports via dlsym(RTLD_DEFAULT)).
 * That puts the ENTIRE m64p API in the global scope, and the m64p plugin ABI
 * defines function-POINTER globals with the same names as the core's exported
 * functions (ConfigGetSharedDataFilepath, ConfigOpenSection, ...). With the core
 * global, plugins loaded afterwards get those variables bound to the core's
 * .text, and PluginStartup's first `ConfigX = dlsym(...)` assignment writes to
 * read-only code -> SIGSEGV before anything renders (proven from the rig core
 * dump, 2026-07-15). The plugin needs no promotion: it resolves CoreVR_* on a
 * handle from dlopen("libmupen64plus.so.2", RTLD_NOLOAD) when RTLD_DEFAULT
 * comes back NULL. */
